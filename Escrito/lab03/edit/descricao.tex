...
\lstset{style=shell}
\lstinputlisting[language=C,firstline=102,lastline=106, label=code:input, caption="Capturação de eventos utilizando a biblioteca \textit{<linux/input.h>}"]{../../labs/driver/usbmouse.c}
...
Para representar o dispostivo propriamente dito, é utilizada a estrutura \textit{struct input\_dev}. Nela são definidos alguns campos responsáveis pela ligação dos eventos definidos anteriormente com o dispositivo físico. O código a seguir extraído de um exemplo de driver de mouse usb, fora modificado para que pudesse ser melhor explicado.
\lstset{style=shell}
\lstinputlisting[language=C,firstline=181,lastline=191, label=code:input_dev, caption="Processo de ligação de um device com os eventos definidos no código \ref{code:input}]{../../labs/driver/usbmouse.c}
